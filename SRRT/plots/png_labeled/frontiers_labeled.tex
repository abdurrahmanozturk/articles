...
Nuclear reactors have a challenging environment for materials due to ionizing radiation originating from fission events. Ionizing radiation can be categorized into two types, charged particle radiation and neutral radiation. The charged particles are variety of energetic particles, such as electrons, protons, and ions. The neutral radiations are photons and neutrons. Among these, the neutrons cause the most significant damage in reactor components.

Reactor materials are not able to fully tolerate radiation damage, consequently they experience microstructural changes that result in degradation of physical, mechanical, and thermal properties. Some of the main consequences of microstructural changes are radiation embrittlement, irradiation creep, irradiation growth, void swelling, and high temperature helium embrittlement. Irradiation effects on materials need to be understood to predict their performance and make provision against potential problems that can prevent operation of a nuclear reactor. \cite{Vanderlaan2012, English2003, ryabikovskaya2021, yingling2021}

To understand effect of irradiation on materials, first, it is needed to figure out what the radiation damage is on atomic scale and what kind of micro-structural changes radiation causes. Conducting this kind of research requires a reactor environment which has naturally high radioactivity level and is not always available for material irradiation. Instead, energetic particles, such as electrons, protons, and ions, can be used to emulate neutron irradiation of reactor components. Furthermore, these energetic particles can create the same amount of neutron radiation damage in a brief period. \cite{shao2017, sun2017, krasheninnikov2010, was2014, zhang2018}.

Was et al. examined 304SS and 316SS cladding steels samples to show whether proton irradiation can have comparable properties with neutron irradiation. They irradiated samples with neutrons at 275C and protons at 360C. Due to dose rate differences between neutron and proton irradiation, they examined samples at different temperatures. As a result of their investigations, radiation-induced segregation (RIS) behavior at the grain boundary and the defect kinetics were in excellent agreement. They observed that proton irradiation mimic neutron irradiation \cite{was_emulation_2002}.

Radiation damage is defined as the displacement of an atom from its lattice site by transfer of kinetic energy from the incident energetic particle. The displaced atom can move through crystal structure, colliding with other lattice atoms and displacing them as well, and create damage cascade. The numerous atomic collisions create vacant sites and displaced atoms sitting on interstices of crystal lattice. These fundamental defects are known as point defects, which are vacancies and interstitials. The point defects can diffuse through material, recombine with each other, and react with sinks that are present in microstructure. The numerical model of point defects can be solved using spatially resolved rate-theory (SRRT) which requires solving coupled differential equations that each represents different defect type \cite{bennett1975, warburton1975, franklin1975, thompson1974, ullmaier1980}.

There has been significant research on studying SRRT to better understand behavior of materials. \cite{short2016} solved rate theory equation by accounting injected interstitials and investigated how spatial void swelling profile changes. They used a defect production profile calculated by SRIM for ions and observed double peak at swelling curves which is matching experimental data. \cite{chen2019} studied proton irradiation induced dislocation loops in modified 310S steels. They used a series of rate equations for different defect types including self-interstitial atoms (SIAs), vacancies, interstitial clusters, vacancy clusters, dislocation lines and grain boundaries. \cite{choi2020} used rate theory to understand irradiation creep behavior of iron-based alloys exposed to stress-applied environment. They coupled rate theory equations with creep model and analyze the behavior at different dose rates. \cite{saidi2021} presented an approach about how dose rate affects radiation damage accumulation in polycrystalline systems in the presence of internal and grain boundary sink strengths. They used both proton and electron irradiation in their study and calculated the relation between internal sink strength to the grain boundary sink strength.

The previous efforts on solving rate theory equations (i.e point defect equations) mostly focused on effect of irradiation dose rate and type, whereas ignoring the effect of grain size. Alongside these, our work aimed to provide insight into effects of not only grain size, but also multiple parameters on spatially resolved defect concentration profiles. In this manner, we solve rate theory equations for two different uniform dose rates, seven different production biases and nine different grain sizes. Hence, this study explores the effect of different parameters on behavior of point defects. We present a detailed investigation of the effect of grain size on the segregation of point defects to interfaces and the resultant nucleation of voids.
...
To validate the numerical implementation of the balance equations of point defects in MOOSE, the solution to a simple well-known benchmark problem is reproduced first. In this problem, analytical solutions to the point defect concentrations in a spherical grain with a perfect and neutral boundary/surface are derived. To simplify the derivation, the production bias and mutual recombination are ignored. For such conditions, the steady state analytical solutions for a spherical grain of radius a are found as below \citep{heald1977}.
 ...
 \subsubsection{Concentrations}
    ...
    \begin{figure}[h!]
        \centering
        a)\includegraphics[scale=0.55]{interstitial_concentration_time_evolution_500nm}
        b)\includegraphics[scale=0.55]{vacancy_concentration_time_evolution_500nm}
    ...
    \begin{figure}[h!]
        \centering
        a)\includegraphics[scale=0.55]{interstitial_concentration_5-250nm-high_neutron-0.png}
        b)\includegraphics[scale=0.55]{interstitial_concentration_500-5000nm-high_neutron-0.png}
        c)\includegraphics[scale=0.55]{vacancy_concentration_5-250nm-high_neutron-0.png}
        d)\includegraphics[scale=0.55]{vacancy_concentration_500-5000nm-high_neutron-0.png}
    ...
    \begin{figure}[h!]
        ...
        b)\includegraphics[scale=0.55]{interstitial_concentration_500-5000nm-high_neutron-5}
        c)\includegraphics[scale=0.55]{vacancy_concentration_500-5000nm-neutron-5}
   ...
   \begin{figure}[htb!]
        \centering
        a)\includegraphics[scale=0.3]{data_neutron_50nm}
        b)\includegraphics[scale=0.3]{data_high_neutron_50nm}
        c)\includegraphics[scale=0.3]{data_neutron_1000nm}
        \qquad
        d)\includegraphics[scale=0.3]{data_high_neutron_250nm}
...
\subsubsection{Vacancy Supersaturation}
    ...
    \begin{figure}[h!]
        ...
        b)\includegraphics[scale=0.55]{super_saturation_500-5000nm-high_neutron-5}
        \qquad
        c)\includegraphics[scale=0.35]{data_neutron_Sv_center}
        d)\includegraphics[scale=0.35]{data_high_neutron_Sv_center}
    ...
    \end{figure}
        ...
        \begin{figure}[h!]
        \centering
        a)\includegraphics[scale=0.55]{center_supersaturation_change_fitted.png}
        \\
        \qquad
        b)\includegraphics[scale=0.55]{center_supersaturation_change_neutron_5_nanograins_fitted.png}
        c)\includegraphics[scale=0.55]{center_supersaturation_change_neutron_5_micrograins_fitted.png}
...
\subsubsection{Grain Boundary Sink Strength}
    ...
    \begin{figure}[h!]  %Sink Strength - Neutron 1e-6 dpa/s - 5% Bias rate
        ...
        \qquad
        c)\includegraphics[scale=0.55]{sink_strength_moose_neutron_5_k_scaled}
        d)\includegraphics[scale=0.55]{sink_strength_moose_high_neutron_5_k_scaled}
    ...
    \begin{figure}[h!]  %GB Sink Strength - Neutron 1e-6 dpa/s
        \centering
        \includegraphics[scale=0.3]{legend2}\\
        a)\includegraphics[scale=0.5]{sink_strength_neutron_Zi_scaled_nolegend}
        b)\includegraphics[scale=0.5]{sink_strength_high_neutron_Zi_scaled_nolegend}
        % \includegraphics[scale=0.35]{legend}
        c)\includegraphics[scale=0.5]{sink_strength_neutron_Zv_scaled_nolegend}
        d)\includegraphics[scale=0.5]{sink_strength_high_neutron_Zv_scaled_nolegend}
        % \includegraphics[scale=0.35]{legend}
    ...
    \centering
        \includegraphics[scale=0.3]{legend2}\\
        a)\includegraphics[scale=0.55]{sink_strength_neutron_ki_scaled_nolegend}
        b)\includegraphics[scale=0.55]{sink_strength_high_neutron_ki_scaled_nolegend}
        % \includegraphics[scale=0.35]{legend}
        c)\includegraphics[scale=0.55]{sink_strength_neutron_kv_scaled_nolegend}
        d)\includegraphics[scale=0.55]{sink_strength_high_neutron_kv_scaled_nolegend}
        % \includegraphics[scale=0.35]{legend}
...
\clearpage
    \begin{figure}[h!]  %Sink Strength 3D - Neutron 1e-6 dpa/s
        ...
        b)\includegraphics[scale=0.55]{sink_strength_moose_neutron_3D_Zv_scaled}
...
%In the above, the term on the left-hand side of Eq.\ref{equation:RIS_equations} represents the space-time evolution of the concentration of interstitials ${C_i}$, concentration of vacancies ${C_v}$, and concentration of Cr atoms ${C_{Cr}}$. On the right-hand side, first term ${\nabla\cdot J_i}$ , ${\nabla\cdot J_v}$ and ${\nabla\cdot J_{Cr}}$ are the divergences of the interstitial, vacancy and Cr atoms fluxes. Second term is a source term representing defect generation with rate of $K_0$. Third term ${K_{iv}C_iC_v}$ is a reaction term, referring the recombination process if the defect $x$ reacts with defect $y$ with a rate of ${K_{xy}}$. (e.g. ${K_{iv}}$ is referring the interstitials and vacancies recombination rate.) The last term is also a reaction term, representing the reaction between the defect $x$ and the sink $s$ with a rate of ${K_{xs}}$.
...
We simulated radiation induced segregation model at an irradiation temperature of 773 $K$, the most common temperature used for RIS studies \citep{allen_thermal_2007}, \citep{wharry_systematic_2013}, \citep{barnard_ab_2014}, \citep{barr_grain_2015} and dose rates of 7e-6 and 1e-3 $dpa/s$. In one of simulations, temperature was varied to 473 $K$ and 1073 $K$ to study its effect. Vacancy and interstitial concentrations were determined as the concentrations at thermal equilibrium for the initial conditions of system. Ni and Cr concentrations are taken as the nominal value in the alloy composition. We assumed that the alloy atoms are randomly distributed throughout the grain at the initial time and bulk sinks are uniformly distributed with a concentration of 1e18 $m^{-3}$. We conducted several simulations to investigate the effects of size, production bias, dose rate, and temperature on RIS in Ni-5Cr. In all of these simulations, the boundary is assumed neutral and perfect, and zero flux boundary condition was used for Cr. Because of the problem symmetry, only one boundary (at x = 0 in the figures) is considered while at the other far end (representing the grain center), zero flux boundary conditions are considered for point defects and Cr. The equations were solved numerically with the initial and boundary conditions, and one-dimension calculations were performed.
%We simulated radiation induced segregation model at an irradiation temperature of 773$K$, the most common temperature used for RIS studies \citep{allen_thermal_2007}, \citep{wharry_systematic_2013}, \citep{barnard_ab_2014}, \citep{barr_grain_2015} and dose rate of 1e-3 $dpa/s$. In one of simulations, temperature was varied to 473$K$ and 1073$K$ to study its effect. Vacancy and interstitial concentrations were determined as the concentrations at thermal equilibrium for the initial conditions of system. To ensure imbalance between production of vacancies and interstitials, percent production bias ( 5\%, 10\%, 20\%)  is used in the vacancy equation. Ni and Cr concentrations are taken as the nominal value in the alloy composition. We assumed that the alloy atoms are randomly distributed throughout the grain at the initial time and sinks are uniformly distributed with a concentration of 1e18 $m^{-3}$. The equations were solved numerically with the initial and boundary conditions, and one-dimension calculations were performed.
...
%We conducted several simulations to investigate the effects of size, production bias, dose rate, and temperature on RIS in Ni-5Cr. In all of these simulations, the boundary is assumed neutral and perfect, and zero flux boundary condition was used for Cr. Because of the problem symmetry, only one boundary (at x = 0 in the figures) is considered while at the other far end (representing the grain center), zero flux boundary conditions are considered for point defects and Cr. Our model predictions for the case of large grains and without bias agree well with the numerical results and experimental data presented in \citep{barr_grain_2015}. However, we demonstrate here a strong dependence of RIS on size and bias, which were not explored in \citep{barr_grain_2015}. The quantitative effects of size and production bias on RIS in Ni-5Cr are presented in Figs. \ref{figure:RIS_773K_0vs5vs10vs20} and \ref{figure:RIS_Cr_50_125_250_500nm_773K}. 

%In Fig. \ref{figure:RIS_773K_0vs5vs10vs20}, the effect of production bias is shown for two different grain sizes at a temperature of 773 $K$ and a dose rate of 1e-3 $dpa/s$. As evident from the figure, the depletion of Cr at the boundary becomes more pronounced as the production bias increases. Both the boundary Cr concentration is reduced, and the depletion layer is widened with bias. This is consistent with the fact that Cr exchanges preferably with vacancies, which rise with bias. However, it is noteworthy that the effect of bias is more apparent in the large grain. For the 500 $nm$ grain with 10\% production bias, the steady-state vacancy concentration is one order of magnitude higher than its corresponding value in the absence of bias. Conversely, the steady-state interstitial concentration is one order of magnitude lower than its corresponding value in the absence of bias.  This is because of the size-dependent instability discussed before, i.e., the instability is only triggered in the large grain. 

%In Fig. \ref{figure:RIS_Cr_50_125_250_500nm_773K},  The effect of size on RIS in Ni-5Cr is explicitly investigated for two different dose rates in the presence and absence of bias. Consistent with the results from section 3.1, the size effect saturates above 250 $nm$ in the absence of bias. Nonetheless, in the presence of bias, the depletion of Cr increases with size. Moreover, the rate of increase rises with size. It is also clear that the strength of depletion and its size dependence are amplified at higher dose rates. At the low dose rate of 7e-6 $dpa/s$, the model predictions lie within the results obtained before \citep{barr_grain_2015}, where Cr concentration at the boundary reported to be between 2-4\%. Nonetheless, the predictions here for 20\% bias show inverse dependence on grain size, i.e., Cr concentration at the boundary becomes smaller with increasing size. Moreover, at the high dose rate of 1e-3 $dpa/s$, the Cr boundary concentration becomes negligibly small at sizes above 250 $nm$. However, in this case, the treatment of the boundary as a perfect sink is probably no longer valid.  
...
the instability is absent (see Fig. \ref{figure:RIS_473vs773vs1073}a). However, it could still happen at this temperature for higher dose rate, higher production bias, or larger size. Hence, RIS is least observed at the highest temperature. As the temperature decreases, RIS becomes more apparent with higher segregation of Ni and depletion of Cr at the boundary as the production bias increases. RIS is predicted here to be maximum at the lowest temperature of 473 $K$, while it is often reported to be maximum at intermediate temperature \citep{barnard_ab_2014}, \citep{barr_grain_2015}. This could be attributed to the uncertainties associated with interstitial diffusivities at lower temperature and the validity of the perfect sink assumption, as was discussed in \citep{barnard_ab_2014}. Nevertheless, the combined effect of size and production bias on RIS is clear.

%Lastly, the effect of temperature on RIS in Ni-5Cr was investigated. Fig. \ref{figure:RIS_473vs773vs1073} summarizes the results of simulations conducted for a 500 $nm$ grain irradiated at 1e-3 $dpa/s$ with two different production biases. First, the dependence of point defects on temperature is obvious, i.e., as temperature increases, the steady-state concentration of vacancies decreases, while the interstitial counterpart rises. Therefore, the difference between the point defect concentrations is reduced at high temperature. Moreover, at the highest temperature of 1073 $K$, the instability is absent (see Fig. \ref{figure:RIS_473vs773vs1073}a). However, it could still happen at this temperature for higher dose rate, higher production bias, or larger size. Hence, RIS is least observed at the highest temperature. As the temperature decreases, RIS becomes more apparent with higher segregation of Ni and depletion of Cr at the boundary as the production bias increases. RIS is predicted here to be maximum at the lowest temperature of 473 $K$, while it is often reported to be maximum at intermediate temperature \citep{barnard_ab_2014}, \citep{barr_grain_2015}. This could be attributed to the uncertainties associated with interstitial diffusivities at lower temperature and the validity of the perfect sink assumption, as was discussed in \citep{barnard_ab_2014}. Nevertheless, the combined effect of size and production bias on RIS is clear. 

     \begin{figure}[h!]  %
        \centering
        a)\includegraphics[scale=0.55]{srrt/plots/RIS_Xv_50nm_773K_0vs5vs10vs20.png}
        b)\includegraphics[scale=0.55]{srrt/plots/RIS_Xv_500nm_773K_0vs5vs10vs20.png}
        \\
        c)\includegraphics[scale=0.55]{srrt/plots/RIS_Xi_50nm_773K_0vs5vs10vs20.png}
        d)\includegraphics[scale=0.55]{srrt/plots/RIS_Xi_500nm_773K_0vs5vs10vs20.png}
        \\
        e)\includegraphics[scale=0.55]{srrt/plots/RIS_Cr_50nm_773K_0vs5vs10vs20.png}
        f)\includegraphics[scale=0.55]{srrt/plots/RIS_Cr_500nm_773K_0vs5vs10vs20.png}
...
\clearpage  
    \begin{figure}[h!]  %
        \centering
        a)\includegraphics[scale=0.55]{srrt/plots/RIS_Cr_50_125_250_500nm_773K_1e-3_0.png}
        b)\includegraphics[scale=0.55]{srrt/plots/RIS_Cr_50_125_250_500nm_773K_1e-3_20.png}
        c)\includegraphics[scale=0.55]{srrt/plots/RIS_Cr_50_125_250_500nm_773K_7e-6_0.png}
        d)\includegraphics[scale=0.55]{srrt/plots/RIS_Cr_50_125_250_500nm_773K_7e-6_20.png}
        ...
        %\caption{Effect of grain size on the steady-state concentration profiles of Chromium. Upper row: concentration profiles of Chromium at 1e-3 $dpa/s$ and a) 0\% and b) 20\% production bias. Lower row: concentration profiles of Chromium at 7e-6 $dpa/s$ and c) 0\% and d) 20\% production bias. Similar to the case of pure Ni (recall Figs. 4 and 5), size effect saturates above $\sim$ 250 $nm$ in the absence of bias, while it continues to become stronger in the presence of bias. The depletion of Cr increases with bias and size. This increase is more pronounced at higher dose rates.   }
    ...
    \begin{figure}[h!]  %
        \centering
        a)\includegraphics[scale=0.55]{srrt/plots/RIS_Xi_500nm_473vs773vs1073_5_20_K.png}
        b)\includegraphics[scale=0.55]{srrt/plots/RIS_Xv_500nm_473vs773vs1073_5_20_K.png}
        c)\includegraphics[scale=0.55]{srrt/plots/RIS_Cr_500nm_473vs773vs1073_5_20_K.png}
        d)\includegraphics[scale=0.55]{srrt/plots/RIS_Ni_500nm_473vs773vs1073_5_20_K.png}
        e)\includegraphics[scale=0.55]{srrt/plots/RIS_Cr_500nm_473vs773vs1073_5_20_closeup_K.png}
        f)\includegraphics[scale=0.55]{srrt/plots/RIS_Ni_500nm_473vs773vs1073_5_20_closeup_K.png}
