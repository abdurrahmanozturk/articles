...
\end{equation}
    ...
    \end{equation}
    where $r_{xy}$ and $r_{xs}$ are interaction radii for the reaction of defect \textit{x} with defect \textit{y} or a sink \textit{s}.
...
\subsection{Non-dimensionalization of Balance Equations for Point Defects}
    ...
    \begin{equation}
      \begin{aligned}
        &\widetilde{k_{i}^2}=\frac{l^2 k_{i}^2}{\Omega}\\
        &\widetilde{k_{i}^2}=\frac{l^2 k_{v}^2}{\Omega}
      \end{aligned}
...
\begin{equation}
    \begin{aligned}
    &J_{V} = d_{CrV}\Psi\Omega C_{V}\nabla C_{Cr} + d_{NiV}\Psi\Omega C_{V}\nabla C_{Ni} - D_{V}\nabla C_{V}\\
    &J_{I} = -d_{CrI}\Psi\Omega C_{I}\nabla C_{Cr} + d_{NiI}\Psi\Omega C_{I}\nabla C_{Ni} - D_{I}\nabla C_{I}\\
    &J_{Cr} = -D_{Cr}\Psi\nabla C_{Cr} + d_{CrV}\Omega C_{Cr}\nabla C_{V} - d_{CrI}\Omega C_{Cr}\nabla C_{I}\\
    \end{aligned}
    \label{equation:RIS_equations}
 \end{equation}
     
The time derivatives in Eq.\ref{equation:RIS_equations} represent the evolution of the concentration of interstitials ${C_i}$,  concentration of vacancies ${C_v}$,  and concentration of Cr atoms ${C_{Cr}}$.   On the right-hand side, first terms ${\nabla\cdot J_i}$ , ${\nabla\cdot J_v}$ and ${\nabla\cdot J_{Cr}}$ are the divergences of the interstitial, vacancy and Cr atoms fluxes. The geometry is represented in Cartesian coordinates in MOOSE model. The exact expressions for the fluxes are given in \citep{barr_grain_2015}. The second term is a source term representing defect generation with rate constant of $K_0$. Third term ${K_{iv}C_iC_v}$ is a reaction term, referring to the recombination between interstitials and vacancies. The last term is also a reaction term, representing the reaction between the defect $x$ and the sink $s$ with a rate constant of ${K_{xs}}$.
